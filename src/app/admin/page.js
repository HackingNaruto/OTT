...
export default function Admin() {
  ...
  const [activeTab, setActiveTab] = useState('cms'); // 'cms' or 'settings'
    ...
    <div className="min-h-screen bg-zinc-950 text-white p-6">
      <div className="max-w-4xl mx-auto bg-zinc-900 border border-zinc-800 p-8 rounded-2xl shadow-xl">
        ...
        <div className="flex gap-4 mb-8 border-b border-zinc-800 pb-4">
          <button onClick={() => setActiveTab('cms')} className={`px-4 py-2 font-bold rounded-lg transition ${activeTab === 'cms' ? 'bg-red-600 text-white' : 'text-zinc-400 hover:bg-zinc-800'}`}>Content CMS</button>
          <button onClick={() => setActiveTab('settings')} className={`px-4 py-2 font-bold rounded-lg transition ${activeTab === 'settings' ? 'bg-red-600 text-white' : 'text-zinc-400 hover:bg-zinc-800'}`}>Player Settings</button>
        </div>
        ...
        {activeTab === 'cms' ? (
          ...
          <form onSubmit={handleSaveSettings} className="space-y-6">
            <div className="bg-black p-6 rounded-xl border border-zinc-800 space-y-6">
              <h3 className="text-lg font-bold border-b border-zinc-800 pb-2">Global Watermark</h3>
              ...
              {watermarkEnabled && (
                <div className="space-y-2">
                  ...
                  <input type="text" placeholder="E.g., StreamX Protected" className="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-3 text-sm focus:border-red-600 outline-none" value={watermarkText} onChange={e => setWatermarkText(e.target.value)} required />
                </div>
              )}
            </div>
